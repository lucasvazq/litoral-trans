...
export const Header = ({breadcrumb: string}) => {
    ...
    return (
        <nav ref={navBar} className="bg-primary text-light fixed shadow-md top-0 w-full z-2">
        <div className="m-8">
          <div className="flex justify-between">
            ...
            <button
              ...
              className="text-3xl flex items-center h-8 w-8"
            >
          ...
          <div ref={hiddenMenu} className={`${menuIsActive ? "opacity-100" : "opacity-0"} mt-6 mb-1`}>
            <ul className="list-reset flex-1 justify-end items-center text-sm">
              <li>
                <Route href={`mailto:${process.env.email}`} icon={MdEmail} description={process.env.email} />
              </li>
              <li>
                <Route href={`tel:${process.env.telCountryCode}${process.env.telAreaCode}${process.env.telPhoneNumber}`} icon={HiPhone} description={`${process.env.telCountryCode} ${process.env.telAreaCode} ${process.env.telPhoneNumber}`} />
              </li>
            </ul>
          </div>
        </div>
